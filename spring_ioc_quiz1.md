...
1. 위 두 클래스를 사용하는 **스프링 구성 클래스(AppConfig)** 를 작성하세요.
...
2. 스프링 구성 파일(AppConfig 또는 어노테이션 방식)에서

   * 처음에는 `FixDiscountPolicy`를 주문 서비스에 주입하도록 구성
   * 나중에 **코드 수정 없이 구성만 바꿔서** `RateDiscountPolicy`를 사용하도록 변경 가능하게 작성
...
1. 위 컴포넌트들을 **자동 스캔**해서 등록하는 구성 클래스를 작성하세요.
